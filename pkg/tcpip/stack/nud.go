...
// NUDConfigurations.
func (c NUDConfigurations) resetInvalidFields() NUDConfigurations {
...
// NewNUDState returns new NUDState using c as configuration.
func NewNUDState(c NUDConfigurations) *NUDState {
	s := &NUDState{
...
func (s *NUDState) ReachableTime() time.Duration {
	...
	if s.mu.config.BaseReachableTime != s.mu.prevBaseReachableTime {
		return s.recomputeReachableTimeLocked()
	}
	if s.mu.config.MinRandomFactor != s.mu.prevMinRandomFactor {
		return s.recomputeReachableTimeLocked()
	}
	if s.mu.config.MaxRandomFactor != s.mu.prevMaxRandomFactor {
		return s.recomputeReachableTimeLocked()
	}
...
func (s *NUDState) recomputeReachableTimeLocked() time.Duration {
	...
	if randomFactor > 1 && float32(math.MaxInt64)/randomFactor < float32(s.mu.config.BaseReachableTime) {
