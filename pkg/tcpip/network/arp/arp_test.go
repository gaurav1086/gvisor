...
func expectEvents(t *testing.T, d *arpDispatcher, events []eventInfo) {
	...
	for len(d.C) != 0 {
		// Receiving additional events is an error since it invalidates an
		// assumption that only the specified events are dispatched after a certain
		// event in the NUD state machine. No more, no less.
		t.Errorf("unexpected event: %s", <-d.C)
	}
	if t.Failed() {
		t.FailNow()
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if !test.isValid {
				...
				// The test cannot wait forever, so 100ms of waiting time seems
				// reasonable. Sleep tests are gross, since this can false-negative
				// (i.e. the test should fail but won't) if there's a bug. However if
				// there is no bug, this will reliably succeed.
